\documentclass[Master,ngerman,UKenglish, palatino, biblatex = true, Submit]{scrbook}
%------------------------------------------------------------------------------
...
\newcommand{\thesisabstract}{%
  ...
  Neural networks are widely used for signal to background separation in high energy collider physics. Neural networks trained on Monte Carlo simulations are highly sensitive to systematic uncertainties. A proposed techique to diminish this sensitivity is an adversarial neural network consisting of two networks that are trained against each other. The first network tries to separate between signal and background while the second network tries to separate between a nominal signal sample and a systematic signal sample. A Minimax decision rule is used to achieve a good signal to background classification for the first network and a bad nominal to systematics for the second network. In this work an adversarial neural network trained on \tW dilepton channel Monte Carlo simulations with \ttbar background using the ATLAS detector is introduced. Testing and tuning of the hyperparameters is presented for both networks as well as three different approaches to the setup of the adversarial neural network.
  \end{otherlanguage}
}
